...
```bash
# Both queries need --include-infra: wisp roots are ephemeral, so gc bd list
# hides the wisps tier without it. Without the flag CURRENT_WISP resolves
# empty and the current wisp leaks, and ASSIGNED_WISP reports "no successor
# queued" even when one is — so this fallback pours a duplicate.
